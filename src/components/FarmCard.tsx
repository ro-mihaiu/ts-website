...
import { Download, FileCode, Youtube, ExternalLink, Sparkles, Layers, ArrowUpRight, Gauge, Eye } from "lucide-react";
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5 z-10">
          <span
            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold tracking-wide uppercase font-mono shadow-md backdrop-blur-md ${
              isJava
                ? "bg-amber-500/20 text-amber-300 border border-amber-500/40 bg-slate-950/80"
                : "bg-rose-500/20 text-rose-300 border border-rose-500/40 bg-slate-950/80"
            }`}
          >
            {isJava ? <Layers className="w-3 h-3" /> : <Sparkles className="w-3 h-3" />}
            {isJava ? "Java" : "Bedrock"}
          </span>
        ...
        <div className="pt-3 border-t border-slate-800/80 flex items-center gap-2">
          ...
          {isJava && (
            farm.hasSchematic && farm.schematicUrl ? (
              ...
              <Link
                href={farmUrl}
                className="px-2.5 py-2 rounded-xl text-[11px] font-medium text-slate-500 bg-slate-900/80 border border-slate-800/80 hover:text-amber-400 hover:border-slate-700 transition-colors"
